...
// 详情页面动态绑定
$(function(){
    var bid=location.search.split("=")[1];//截取id
    $.ajax({
        url:"http://localhost:5050/public/shopping3",
        data:{bid},
        type:"get",
        datatype:"json",
        success:function(result){ 
            ...
            $("#box2").html(sp_4);
            // 加入购物车
            $(".buy").click(function(bid,title,img,count,price){ 
                var price=shop_0.price;
                var title=shop_0.title;
                var bid=shop_0.bid;
                var img=shop_0.img;
                var count=1;  
                console.log(price,count,title,bid,img)
                $.ajax({
                    url:"public/addcart", 
                    data:{bid,title,img,count,price},
                    type:"get",
                    dataType:"json",
                    success:function(result){
                        if(result==1){
