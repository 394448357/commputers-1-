...
search.onclick=function(){
  var title=inp.value;
  console.log(title);
  window.location.href="http://localhost:5050/b_computer.html?title="+`${title}`;
}
$(function(){  
    var title=decodeURI(location.search.split("=")[1]);
    console.log(title);
  $.ajax({
    url:"http://localhost:5050/public/searcher2",
    type:"get",
    dataType:"json",
    async: false,
    data:{title},
    success:function(result){
      console.log(result);
      var str1="";
      for ( var ss1 of result){
       console.log(ss1);
         str1+=`<div class="inner">
             <a href="${ss1.href}">
                 <img src="${ss1.img}" alt="">
                 <p class="co_1">${ss1.title}</p> 
             </a>
                 <p class="co_2">${ss1.details}</p>
                 <p class="co_3">¥${ss1.price}</p>
         </div>`;
      }
      for (var i=0;i<=10;i++){
        $(".bjb_2").append(str1);
